...
int main(int argc, char *argv[]){
	...
	for (int i = 1; i < argc; i++){
		...
		else if (str_equal(argv[i], "-p") || str_equal(argv[i], "--listen-port")){
			i++;
			if (argc == i){
	...
	while (true){
		...
		if (verbosity >= VERBOSITY_INFO){
			inet_ntop(listen_ip_family, listen_ip_family == AF_INET ? &sock_client.ipv4.sin_addr.s_addr : (in_addr_t*)&sock_client.ipv6.sin6_addr, packet + 100, (sizeof packet) - 100);
